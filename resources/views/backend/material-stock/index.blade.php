...
@section('content')
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered text-nowrap" id="material_stock_table">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Stock Date</th>
    ...
    <div class="modal fade bd-example-modal-lg" id="material_stock_form_modal" tabindex="-1" role="dialog" aria-hidden="true">
...
@section('js')
    ...
    <script>
        $(document).on('click', '#add_new', function() {
            $.ajax({
                url: "{{ route('material-stock.create', ['username' => $siteSlug]) }}",
                type: "GET",
        ...
        var table = $('#material_stock_table').DataTable({
            processing: true,
            serverSide: true,
            responsive: true,
            ajax: "{{ route('material-stock.index', ['username' => $siteSlug]) }}",
            columns: [
                { data: 'id', name: 'id' },
        ...
        $(document).on('click', '.delete-material-stock', function() {
            var id = $(this).attr("data-id");
            swal({
                title: "Are you sure?",
                text: "Once deleted, you will not be able to recover this material stock!",
                icon: "warning",
                buttons: true,
                dangerMode: true,
                ...
                if (willDelete) {
                    $.ajax({
                        ...
                        url: "{{ route('material-stock.destroy', ['username' => $siteSlug, 'material_stock' => ':id']) }}".replace(':id', id),
                        success: function(response) {
                            swal({
                                title: response.msg
                            }, function(result) {
