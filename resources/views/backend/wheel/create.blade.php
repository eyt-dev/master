...
<div class="container">
    ...
    @endif
    <h2>Create Wheel</h2>
    <form action="{{ route('wheel.store', ['username' => $siteSlug]) }}" method="POST" novalidate class="needs-validation">
        @csrf
...
$(document).ready(function() {
    ...
    $('#game_id').change(function() {
        ...
        if (gameId) {
            $.ajax({
                url: "{{ route('getClipsByGame', ['username' => $siteSlug]) }}", // Route to fetch clips
                type: "GET",
                data: { game_id: gameId },
